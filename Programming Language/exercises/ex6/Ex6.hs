...
--      Don't get distracted trying to "turn" `"(+ 3 4)"` into "7".
...
cumulateExpr :: Int -> [String]
cumulateExpr k = 
    if k == 0
        then numbers
        else (expressionsOfRank k) ++ (expressionsOfRank (k-1))
...
expressionsOfRank k = 
    if k == 0 
        then numbers
            ...
            if k == 1
            then
                [makePlus  x y | x <- (expressionsOfRank (k-1)), y <- (expressionsOfRank (k-1))] ++
                [makeTimes x y | x <- (expressionsOfRank (k-1)), y <- (expressionsOfRank (k-1))]
            else
                [makePlus  x y | x <- (expressionsOfRank (k-1)), y <- (cumulateExpr (k-2))] ++
                [makePlus  x y | x <- (cumulateExpr (k-2)), y <- (expressionsOfRank (k-1))] ++ 
                [makePlus  x y | x <- (expressionsOfRank (k-1)), y <- (expressionsOfRank (k-1))] ++ 
                [makeTimes x y | x <- (expressionsOfRank (k-1)), y <- (cumulateExpr (k-2))] ++
                [makeTimes x y | x <- (cumulateExpr (k-2)), y <- (expressionsOfRank (k-1))] ++ 
                [makeTimes x y | x <- (expressionsOfRank (k-1)), y <- (expressionsOfRank (k-1))]
...
arithmeticExpressions = foldl (++) [] [expressionsOfRank k | k<-[0..99999]]
-- when I leave [0..], it compile but stucked when trying to excute it
...
-- Precondition: k >= 0.
--
cumulateExprGen :: BinaryGrammar -> Int -> [String]
cumulateExprGen b k = 
    if k == 0
        then (expressionsOfRankGen b k)
        else 
            (expressionsOfRankGen b k) ++ 
            (expressionsOfRankGen b (k-1))
...
expressionsOfRankGen (BinaryGrammar atoms rules) k = 
    if k == 0
        then atoms
            ...
            if k == 1
            then [op1 x y | x<-atoms, y<-atoms] ++ [op2  x y | x<-atoms, y<-atoms]
            else
                [op1 x y | x<-(expressionsOfRankGen b (k-1)), y <- (cumulateExprGen b (k-2))] ++
                [op1 x y | x<-(cumulateExprGen b (k-2)), y <- (expressionsOfRankGen b (k-1))] ++ 
                [op1 x y | x<-(expressionsOfRankGen b (k-1)), y <- (expressionsOfRankGen b (k-1))] ++ 
                [op2  x y | x<-(expressionsOfRankGen b (k-1)), y <- (cumulateExprGen b (k-2))] ++
                [op2  x y | x<-(cumulateExprGen b (k-2)), y <- (expressionsOfRankGen b (k-1))] ++ 
                [op2  x y | x<-(expressionsOfRankGen b (k-1)), y <- (expressionsOfRankGen b (k-1))]
    where 
        b = (BinaryGrammar atoms rules)
        op1 = head rules
        op2 = last rules
...
expressions (BinaryGrammar atoms rules) = 
    foldl (++) [] [expressionsOfRankGen b k | k<-[0..99999]]
    where
        b = (BinaryGrammar atoms rules)
