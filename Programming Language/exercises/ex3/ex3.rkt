#lang racket #| CSC324 Winter 2018: Exercise 3 |#
#|
...
#|
(build-env bindings)
  bindings: a list of Racket data representing bindings.
  ...
  Don't use mutation! Lookup how to use the `hash` function.
...
|#
(define (make-hash expr init-hash)
    (if (list? expr)
        (if (equal? (length expr) 3)
            (let ([action (first expr)]
                  [key (second expr)]
                  [val (if (number? (third expr)) (third expr) (hash-ref init-hash (third expr)))])
              (cond
                [(not (equal? action 'define)) 'fail]
                [(null? init-hash) (hash key val)]
                [else (hash-set init-hash key val)]))
              init-hash)
     init-hash))

(define (build-env bindings)
    (foldl make-hash null bindings))
    
(module+ test
  (require rackunit)
  (test-equal? "Fail bindings"
...
(define fix-first x f)
  ...
  Return a new function g that takes one fewer argument than f,
  such that
  ...
  to define it. Instead, review *rest arguments* and *apply* to complete
  the function. (Lab 3 has a simple "rest argument and apply" exercise.)
...
Notes and hints:
  ...
  3. Use fix-first as a convenient helper.
|#
(define (curry-n n f)
  (lambda (x)
    (if (> n 1) (curry-n (- n 1) (fix-first x f))
        (f x))))
