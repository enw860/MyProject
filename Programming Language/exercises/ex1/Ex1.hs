...
module Ex1 (celsiusToFarenheit, nCopies, numEvens, numManyEvens) where
...
-- But noted as above, you can later try simplifying this definition using
...
numEvens numbers =
     ...
     then 0
         ...
         let firstNumber = head numbers
             remainNumbers = numEvens (tail numbers)
         in
             if even firstNumber
             then (1 + remainNumbers)
             else (0 + remainNumbers)
...
numManyEvens listsOfNums =
     ...
     then 0
         ...
         let firstNumberList = head listsOfNums
             remainNumberLists = numManyEvens (tail listsOfNums)
         in
             if (numEvens firstNumberList) > 2
             then (1 + remainNumberLists)
             else (0 + remainNumberLists)
