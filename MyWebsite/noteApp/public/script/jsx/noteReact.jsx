	...
	render(){
		...
		return (
			<div id="NoteBody">
				<div id="result">
					<div id="search">
						<input type="text" placeholder="Searching text" id="searchInput" onChange={this.searchTextHandler}/>
					</div>
					<div id="NoteBlock">
						{notes.map(note=>(
							<div className="briefNote" key={"note_"+note.id}>
								<div className="noteTag" onClick={() => this.noteSwitch(note)}>
									<span>Title: {coverContext(note.title, 13)}</span><br/>
									<span>Content: {coverContext(note.content, 10)}</span>
								</div>
								<div onClick={() => this.deleteNote(note.id)} className="idClose">X</div>
							</div>
						))}
						{isNotSearching? 
							(<div className="briefNote" onClick={()=>this.createNote("","")}>
								<a id="newNote">+ NEW NOTE</a>
							</div>):
							(<div className="briefNote" onClick={this.endOfSearchHandler}>
								<a>End of search</a>
							</div>)
						}
					</div>
				...
				<div id="NoteContent">
					<input type="text" placeholder="Title" id="Note_Title" onChange={this.titleOnChange}/><br/>
 				    <div id="contentwrapper">
 				    	<textarea placeholder="Context" id="Note_Content" rows="25" onChange={this.contentOnChange}/><br/>
					</div>
					<button id="save_button" onClick={this.saveHandler}>Save</button><br/>
				</div>
			</div>
		);
	};
